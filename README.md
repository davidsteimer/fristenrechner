...
> **Projektstatus:** AP10 hat den abgenommenen AP8-Rechenkern und die AP9-MVP-Oberfläche in ein gemeinsames SPFx-WebPart für SharePoint und Microsoft Teams integriert. Der Kandidat `0.1.0.0` wurde im steimer.ch-Tenant-App-Katalog aktiviert, auf der dedizierten Testsite installiert und fachlich erfolgreich geprüft. Die responsive Prüfung deckte eine SPFx-CSS-Lokalisierung auf, welche das Produktlayout im Host ausser Kraft setzte. Der erste Korrekturversuch `0.1.0.1` wurde ebenfalls im Tenant installiert, liess den Sass-Marker `:global` aber wörtlich im ausgelieferten CSS stehen. Der neue Kandidat `0.1.0.2` ist reproduzierbar gebaut und paketiert. Ein verpflichtender Nachtest des finalen Bundles prüft nun sowohl lokalisierte Klassennamen als auch wörtliche `:global`-Selektoren. Alle 65 bestehenden TypeScript-Tests und alle 12 SPFx-Provider- und Integrationsprüfungen bestehen. Die Tenant-Aktualisierung auf `0.1.0.2`, die responsive Wiederholungsprüfung und die Teams-Tests stehen noch aus. Der Teams-Mirror und Gastzugriffe bleiben bis zu ihren ausdrücklich vorgesehenen Folgeprüfungen unkonfiguriert beziehungsweise gesperrt. Es besteht noch keine produktiv freigegebene Anwendung.
